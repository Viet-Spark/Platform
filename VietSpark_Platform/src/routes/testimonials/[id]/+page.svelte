...
let modalImageUrl = '';
	...
	onMount(async () => {
		...
		try {
			...
			// Only show publicly visible, approved testimonials
			if (data.moderationStatus !== 'Approved' || data.visibility === 'Private') {
				error = 'This testimonial is not available.';
				return;
			}
...
{:else if error}
	<div class="flex min-h-screen items-center justify-center">
		<div class="max-w-xl text-center">
			...
			<a href="/testimonials" class="text-primary mt-4 inline-block hover:underline">Back to Testimonials</a>
		</div>
	</div>
{:else if !testimonial}
	<div class="flex min-h-screen items-center justify-center">
		<div class="text-center">
			...
			<a href="/testimonials" class="text-primary hover:underline">Back to Testimonials</a>
		</div>
	</div>
{:else}
