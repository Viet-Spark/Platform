<script>
    ...
    async function getProgramAndApplications() {
        ...
        managerApplications = $applications.filter((app => $curProgram.managerApplicantIds.includes(app.id))); 
        filteredApplications.set(managerApplications);  
    }
...
<section class="container mx-auto">
    ...
    {:else}
		<div class="flex flex-col items-center justify-center bg-primary text-white p-4 mb-8">
			<h1 class="text-2xl font-bold">Manager Applications</h1>
			<p>Manage {$curProgram.title} Manager Applications</p>
		</div>
		{#if managerApplications.length === 0}
			<div class="rounded-lg bg-gray-100 p-8 text-center">
				<p class="text-gray-600">There are currently no manager applications.</p>
			</div>
		{:else}
