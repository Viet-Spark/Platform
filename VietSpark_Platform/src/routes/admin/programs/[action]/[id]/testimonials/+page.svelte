<script>
    ...
    import { userData, userLoading, getUser } from '$lib/stores/userStore';
    ...
    async function loadTestimonials() {
        ...
        try {
            ...
            programTestimonials.set($testimonials.filter((testimonial) => ids.includes(testimonial.id)));
            // Resolve submitter names (async) so the template can display them
            const names = {};
            const uniqueSubmitterIds = [...new Set($programTestimonials.map((t) => t.submitterId).filter(Boolean))];
            await Promise.all(
                uniqueSubmitterIds.map(async (id) => {
                    try {
                        const userInfo = await getUser(id);
                        names[id] = userInfo.displayName || userInfo.name || userInfo.email || '—';
                    } catch {
                        names[id] = '—';
                    }
                })
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    {#if testimonial.authorCoverImage}
                                        <img src={testimonial.authorCoverImage} alt={testimonial.authorName} class="w-10 h-10 rounded-full object-cover">
                                    {:else}
                                        <img src={defaultProfile} alt={testimonial.authorName} class="w-10 h-10 rounded-full object-cover">
                                    {/if}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{testimonial.highlight.slice(0, 150) || ""}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
