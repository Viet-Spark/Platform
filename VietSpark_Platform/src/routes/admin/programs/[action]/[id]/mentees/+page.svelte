<script>
	...
	let filteredApplications = writable([]);

    onMount(() => {
        getProgramAndApplications(); 
    })

    async function getProgramAndApplications() {
        await programHandlers.getProgram($curProgram.id);
        menteeApplications = $applications.filter((app => $curProgram.menteeApplicantIds.includes(app.id))); 
        filteredApplications.set(menteeApplications);  
    }
...
<section class="container mx-auto">
    {#if $programLoading || $applicationLoading || loading}
        <div class="flex h-screen items-center justify-center">
            <span>Loading...</span>
        </div>
    {:else}
		<div class="flex flex-col items-center justify-center bg-primary text-white p-4 mb-8">
			<h1 class="text-2xl font-bold">Mentee Applications</h1>
			<p>Manage {$curProgram.title} Mentee Applications</p>
		</div>
		{#if menteeApplications.length === 0}
			<div class="rounded-lg bg-gray-100 p-8 text-center">
				<p class="text-gray-600">There are currently no mentee applications.</p>
			</div>
		{:else}
