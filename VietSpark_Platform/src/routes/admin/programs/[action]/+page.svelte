<script>
    ...
    import { programHandlers, programLoading } from '$lib/stores/programStore';
    ...
    async function handleSubmit() {
        ...
        try {
            // Create a program to get programId first
            let programId = await programHandlers.createProgram({
                title: formData.title
            })
            // Upload cover image
            let coverImageUrl = formData.coverUrl;
			if (formData.coverTempFile) {
				coverImageUrl = await programHandlers.uploadCoverImage(formData.coverTempFile, programId);
			}
			...
			if (formData.imageTempFiles?.length > 0) {
				const newUrls = await programHandlers.uploadImages(formData.imageTempFiles, programId);
            ...
            await programHandlers.updateProgram(programId, dataToSubmit)
