...
export async function POST({ request }) {
	try {
		const { name, email, careerStatus, interestArea } = await request.json();

		if (!name?.trim() || !email?.trim() || !careerStatus?.trim() || !interestArea?.trim()) {
			return json(
				{
					success: false,
		...
		await sheets.spreadsheets.values.append({
			...
			requestBody: {
				values: [[name.trim(), normalizedEmail, careerStatus.trim(), interestArea.trim()]]
			}
		});
